...
STATUS gfx_init() {
  ...
  g_gfx_tools.window = SDL_CreateWindow("Minimal SDL Example", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, WINDOW_WIDTH, WINDOW_HEIGHT, SDL_WINDOW_SHOWN);
  if (g_gfx_tools.window == NULL) {
    SDL_Log("Window could not be created! SDL_Error: %s\n", SDL_GetError());
    return STATUS_SDL;
  }

  g_gfx_tools.renderer = SDL_CreateRenderer(g_gfx_tools.window, -1, SDL_RENDERER_ACCELERATED);
